...
logger.info("GPU IDS: {}".format(args.gpu_ids))
...
if args.dataset == 'imagenet':
    ...
    if args.scratch:
        checkpoint = torch.load(args.scratch, map_location=torch.device("cpu"))
        if args.dataset == 'imagenet':
...
if args.dataset == 'imagenet':
    ...
    with torch.cuda.device(0):
        ...
        logger.info("##### Num params: {}".format(param_counter(model)))
        exit()
...
criterion = LabelSmoothingLoss(classes=1000, smoothing=0.1).cuda()

for epoch in range(args.start_epoch, args.epochs):
    if early_bird_30.early_bird_emerge(model):
        logger.info("[early_bird_30] Find EB!!!!!!!!!, epoch: "+str(epoch))
        if flag_30:
            save_checkpoint({
            'epoch': epoch + 1,
    ...
    if early_bird_50.early_bird_emerge(model):
        logger.info("[early_bird_50] Find EB!!!!!!!!!, epoch: "+str(epoch))
        if flag_50:
            save_checkpoint({
            'epoch': epoch + 1,
            'state_dict': model.state_dict(),
    ...
    if early_bird_70.early_bird_emerge(model):
        logger.info("[early_bird_70] Find EB!!!!!!!!!, epoch: "+str(epoch))
        if flag_70:
            save_checkpoint({
            'epoch': epoch + 1,
            'state_dict': model.state_dict(),
    ...
    if epoch in args.schedule:
        for param_group in optimizer.param_groups:
            param_group['lr'] *= 0.1
    train(epoch, criterion=criterion)
    prec1 = test(model)
...
if args.swa ==True:
    ...
    logger.info('SWA accuracy: {}'.format(prec1))
np.savetxt(os.path.join(args.save, 'record.txt'), history_score, fmt = '%10.5f', delimiter=',')
