...
__all__ = ['ResNet', 'resnet18', 'baseline_resnet18', 'lowrank_resnet18_conv1x1', 'resnet34', 'resnet50', 
            'lowrank_resnet50', 'lowrank_resnet50_conv1x1', 'resnet101',
...
class LowRankBasicBlockConv1x1(nn.Module):
    ...
    #     torch.cuda.synchronize()
    #     conv1_u_start = time.time()
    #     out = self.conv1_u(x)
    #     torch.cuda.synchronize()
    #     conv1_u_dur = time.time() - conv1_u_start

    #     torch.cuda.synchronize()
    #     conv1_v_start = time.time()
    #     out = self.conv1_v(out)
    #     torch.cuda.synchronize()
    #     conv1_v_dur = time.time() - conv1_v_start
    #     logger.info("@@@ Conv 1 U Cost: {}, Conv 1 V Cost: {}".format(conv1_u_dur, conv1_v_dur))
    ...
    #     logger.info("@@@ Inside the Blk, x shape1: {}".format(x.size()))
    #     torch.cuda.synchronize()
    #     conv2_u_start = time.time()
    #     out = self.conv2_u(out)
    #     torch.cuda.synchronize()
    #     conv2_u_dur = time.time() - conv2_u_start

    #     torch.cuda.synchronize()
    #     conv2_v_start = time.time()
    #     out = self.conv2_v(out)
    #     torch.cuda.synchronize()
    #     conv2_v_dur = time.time() - conv2_v_start
    #     logger.info("@@@ Conv 2 U Cost: {}, Conv 2 V Cost: {}".format(conv2_u_dur, conv2_v_dur))
...
class ResNet(nn.Module):
    ...
    # def _forward(self, x):
    #     logger.info("#### Input of ResNet: {}".format(x.size()))
    #     torch.cuda.synchronize()
    #     resnet_conv1_start = time.time()
    #     x = self.conv1(x)
    #     torch.cuda.synchronize()
    #     resnet_conv1_dur = time.time() - resnet_conv1_start
    #     logger.info("#### ResNet Conv1 Cost: {}".format(resnet_conv1_dur))

    #     torch.cuda.synchronize()
    #     resnet_bn1_start = time.time()
    #     x = self.bn1(x)
    #     torch.cuda.synchronize()
    #     resnet_bn1_dur = time.time() - resnet_bn1_start
    #     logger.info("#### ResNet BN1 Cost: {}".format(resnet_bn1_dur))

    #     torch.cuda.synchronize()
    #     resnet_relu1_start = time.time()
    #     x = self.relu(x)
    #     torch.cuda.synchronize()
    #     resnet_relu1_dur = time.time() - resnet_relu1_start
    #     logger.info("#### ResNet ReLU1 Cost: {}".format(resnet_relu1_dur))

    #     torch.cuda.synchronize()
    #     resnet_maxpool1_start = time.time()
    #     x = self.maxpool(x)
    #     torch.cuda.synchronize()
    #     resnet_maxpool1_dur = time.time() - resnet_maxpool1_start
    #     logger.info("#### ResNet MaxPool1 Cost: {}".format(resnet_maxpool1_dur))

    #     logger.info("#### Start delving into Blk1, before Blk1 size: {}".format(x.size()))
    #     #torch.cuda.synchronize()
    ...
    #     logger.info("#### Start delving into Blk2, before Blk2 size: {}".format(x.size()))
    #     x = self.layer2(x)
    ...
    #     logger.info("#### Start delving into Blk3, before Blk3 size: {}".format(x.size()))
    #     x = self.layer3(x)
    ...
    #     logger.info("#### Start delving into Blk4, before Blk4 size: {}".format(x.size()))
    #     x = self.layer4(x)
...
def _lowrank_resnet_conv1x1(arch, block, layers, pretrained, progress, **kwargs):
...
def lowrank_resnet18_conv1x1(pretrained=False, progress=True, **kwargs):
    r"""ResNet-18 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
...
def lowrank_resnet50_conv1x1(pretrained=False, progress=True, **kwargs):
    r"""ResNet-50 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
...
if __name__ == "__main__":
    simulated_input = torch.randn(1, 3, 224, 224)
    #model = resnet50()
    model = lowrank_resnet50_conv1x1()
    #model.eval()
    #_ = model(simulated_input)
